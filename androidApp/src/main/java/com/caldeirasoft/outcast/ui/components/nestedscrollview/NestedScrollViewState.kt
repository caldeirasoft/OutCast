package com.caldeirasoft.outcast.ui.components.nestedscrollview
...
val NestedScrollViewState.headerScrollRatio: Float
    get() =
        when (maxOffset) {
            0f -> 1f
            else -> ((maxOffset + offset) / maxOffset)
                .coerceAtLeast(0f)
        }

val NestedScrollViewState.expandedHeaderAlpha: Float
    get() = (ln(x = headerScrollRatio.toDouble()) + 1).toFloat().coerceIn(0f, 1f)

val NestedScrollViewState.topAppBarAlpha: Float
